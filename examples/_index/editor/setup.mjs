...
export function setupCodeEditor (iframe) {
    ...
    async function setEditorExample (example111, examples111, customContent, createNew, saveExample) {
        ...
        const htmlPath = new URL('./' + example, window.location.href)
        ...
        // console.log(scripts)
        const exampleScript = Array.from(scripts).find(s=>s.type === 'module');
        if(!exampleScript) {
        ...
        const mainSource = hasContent ? './script.js' : sources.find(s=>s.endsWith('/script.ts') || s.endsWith('/script.tsx')) || sources[0]
        if(!mainSource) {
        ...
        const state = {
            ...
            onChange: (model, uris, content, first = false)=>{
                ...
                // replace script
                const existingScript = parsedHtml.querySelector('script[type="module"]');
                if(existingScript) {
    ...
    function toggleCode() {
        ...
        document.querySelector( '.codebar' ).classList.toggle( 'closed' );
        document.getElementById( 'panel' ).classList.toggle( 'closed' );

        const isClosed = document.querySelector( '.codebar' ).classList.contains( 'closed' );
        const url = new URL( window.location );
        if ( ! isClosed ) url.searchParams.set( 'edit', '' );
    ...
    window.addEventListener( 'keyup', ( ev ) => {
        ...
        // ignore if textarea
        if ( ev.target.id === 'filterInput' ) {

            if ( ev.target.value !== '' ) return;

        } else if ( ev.target.tagName === 'TEXTAREA' || ev.target.tagName === 'INPUT' ) return;
        if ( ev.key === '.' ) {
...
function setupPaneSep () {
    ...
    simpleDrag(paneSep, function (el, pageX, startX, pageY, startY, fix) {
        const sidebarW = 0//sidebar.getBoundingClientRect().width
